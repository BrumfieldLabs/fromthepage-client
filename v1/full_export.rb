...
response = `curl -s -H 'Authorization: Token token=#{key}' 'http://localhost:3000/iiif/collections/#{user}'`
...
collections["collections"].each do |collection_block|
  ...
  raw_response = `curl -s -H 'Authorization: Token token=#{key}' -X POST "http://localhost:3000/api/v1/bulk_export/#{slug}?plaintext_verbatim_page=true&plaintext_verbatim_work=true"`
